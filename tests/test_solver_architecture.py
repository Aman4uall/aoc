...
class SolverArchitectureTests(unittest.TestCase):
    ...
    def test_flowsheet_case_uses_solver_packets(self):
        ...
        self.assertTrue(graph.section_ids)
        self.assertTrue(any(node.section_id for node in graph.nodes))
        self.assertTrue(any(node.stream_roles for node in graph.nodes))
        ...
        self.assertTrue(flowsheet_case.sections)
        ...
        self.assertTrue(any(section.section_id.startswith("purification") for section in flowsheet_case.sections))
        if any(separation.side_draw_stream_ids for separation in flowsheet_case.separations):
            self.assertTrue(any(section.side_draw_stream_ids for section in flowsheet_case.sections))
        ...
        self.assertTrue(solve_result.section_status)
        self.assertTrue(any(section_id.startswith("purification") for section_id in solve_result.section_status))
